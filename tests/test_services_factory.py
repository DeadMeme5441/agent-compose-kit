...
def test_session_redis_with_url_instantiates():
    if not _has_module("google_adk_extras"):
        pytest.skip("google-adk-extras not installed")
...
def test_session_database_with_db_url_instantiates():
    ...
    assert _cls_name(svc) in ("SQLSessionService", "DatabaseSessionService")
...
# GCS artifact support removed; only testing extras-backed artifact services
...
def test_memory_unknown_type_raises():
    with pytest.raises(ValidationError):
        MemoryServiceConfig(type="unknown")
